...
#define KEYBOARD_INTERRUPT                                                     \
    4 // key was struck while running ***** will need refinement when I/O
      // implemented

/* shift and rotate helpers */
#define LSL(val, shift) (((shift) >= 16) ? 0 : ((val) << (shift)))
#define LSR(val, shift) (((shift) >= 16) ? 0 : ((val) >> (shift)))
#define ASR_SIMPLE(val, shift) (((int16_t)(val)) >> (shift))
#define ASR(val, shift)                                                        \
    (((shift) >= 16) ? (BIT(val, 15) ? (int16_t)-1 : 0)                        \
                     : (((int16_t)(val)) >> (shift)))
#define ROR(val, shift) (((val) >> (shift)) | ((val) << (16 - (shift))))

/* bit manipulation macros */
#undef BIT // defined elsewhere
...
#define SIGN_EXTEND(val, topbit)                                               \
    (ASR_SIMPLE(LSL(val, 16 - (topbit)), 16 - (topbit)))
...
class CPU {
  ...
  private:
    ...
    uint16_t
        Regs_displayed[NUM_REGISTERS]{}; // what we last sent to the LED array
...
int CPU::Run(bool slow_display) {
    ...
    do {
        ...
        // piece of code to check the serial port for a break, used in two
        // places below
        auto check_serial = [&return_code]() {
            if (Serial.available()) { // break on any key for now *********
                __attribute__((unused)) char dummy =
                    Serial.read(); // just dump the interrupt character
        ...
        case INSTRUCTION_INVALID:
            // Serial.println("Invalid instruction at location %#x instruction
            // is %#x \n", address,instruction);
        ...
        case INSTRUCTION_NOT_IMPLEMENTED:
            // Serial.println("Not implemented instruction at location %#x
            // instruction is %#x \n",address,instruction);
...
uint16_t CPU::do_add(uint16_t a, uint16_t b, int carry_in, bool cr_writeback) {
    ...
    if (cr_writeback) {
        bool carry =
            (ISNEG(a & b) ||                // both operands are negative, or
             (ISNEG(a ^ b) && ISPOS(val))); // exactly one of the operands is
...
// given the condition register and a 4 bit condition field, return true or
// false
static bool test_cond(uint16_t CR, uint16_t cond) {
    ...
    case COND_LS: // Carry clear or Zero set
        if (((CR & CR_C_BIT_MASK) == 0) || (CR & CR_Z_BIT_MASK))
            res = true;
        break;
    case COND_GE: { // Negative set and Overflow set, or Negative clear and
                    // Overflow clear (N==V)
    ...
    case COND_LT: { // Negative set and Overflow clear, or Negative clear and
                    // Overflow set (N!=V)
        auto val = CR & (CR_N_BIT_MASK | CR_V_BIT_MASK);
    ...
    case COND_GT: { // Zero clear, and either Negative set or Overflow set, or
                    // Negative clear and Overflow clear (Z==0 and N==V)
        if ((CR & CR_Z_BIT_MASK) == 0) {
            auto val = CR & (CR_N_BIT_MASK | CR_V_BIT_MASK);
            if (val == 0 || val == (CR_N_BIT_MASK | CR_V_BIT_MASK))
                res = true;
    ...
    case COND_LE: { // Zero set, or Negative set and Overflow clear, or Negative
                    // clear and Overflow set (Z==1 or N!=V)
        if (CR & CR_Z_BIT_MASK)
            res = true;
...
int CPU::Execute() {
    ...
    uint16_t instruction =
        Memory[Regs[PC_REGISTER]]; // Instruction to be executed
    Regs[PC_REGISTER]++;

    // decode the instruction
    const uint16_t opcode =
        BITS_SHIFT(instruction, 15, 11); // top 5 bits are the main opcode
    ...
    case 0b00010: // adc : NZCV d = a + b + carry
        Put_reg(d_reg, do_add(a_val, b_val,
                              Get_reg(CR_REGISTER) & CR_C_BIT ? 1 : 0, true));
        break;
    ...
    case 0b10000 ... 0b10111: { // branch
        const uint16_t cond =
            BITS_SHIFT(instruction, 13, 10); // conditional bits to test
        ...
        if (cond == COND_NV) { // unconditional branches
            ...
            // save the current PC in the LR register. PC should be pointing at
            ...
            if (test_cond(cr, cond)) {
                ...
                // Serial.print("taking branch, offset 0x");
...
int CPU::Boot(void) { // Boot code goes here, called by 'boot' from console
    ...
    static const uint16_t test_app[] = {
        ...
        0x0068, 0x0069, 0x0073, 0x0020, 0x0069, 0x0073, 0x0020,
        0x0061, 0x0020, 0x0074, 0x0065, 0x0073, 0x0074, 0x0000,
    };
...
int Console::Start() {
    ...
    do {
        ...
        // "help"  help command
            ...
            Serial.println("xr - examine register, then prompts for register "
                           "number in hex ");
            Serial.println(
                "xra - examine all registersxra - prints all register ");
            Serial.println(
                "dr - deposit in register, prompt for register and contents ");
            Serial.println("xm - examine memory, prompt location and number of "
                           "locs in hex ");
            Serial.println("dm - deposit memory,prompt location terminate "
                           "input with cntrl  ");
            Serial.println("s - step a single instruction ");
            Serial.println("r or run- run instructions at full speed ");
            Serial.println(
                "rd - run instructions with delay and display between each ");
            Serial.println("b or boot - boot from internal code routine ");
            Serial.println("q or quit - quit the console ");
            Serial.println("wh - write hex, prompt data and address. Future "
                           "design will prompt starting and ending address");
        }
        ...
        // "dm" deposit memory command
            ...
            if (num_args > 2) { // a single value was also supplied
                ...
                Serial.println("CONS> enter consecutive memory values, break "
                               "with return only ");
                int finished = false;
                do {
        ...
        /*// "write_hex command"
              ...
              if (num_args > 1){      // There are 2 args
                Serial.println("The Intel HEX format of %d and %d
           is:",argv[0],argv[1]); convert_hex(argv[0],argv[1]);
              }
...
void Console::Print_a_register(int reg_number) {
    if ((reg_number >= 0) && (reg_number < NUM_REGISTERS)) {
        Serial.print("CONS> Register ");
        Serial.print(reg_number, HEX);
        Serial.print("= ");
        Serial.println(cpu.Get_reg(reg_number), HEX);
    } else {
...
void Console::convert_hex(int value, int address){
  ...
  int data = value;
  int checksum = 00;      // There are functions to calc this, gonna leave it
blank for now


  Serial.println("%c %d %d %d %d %d \n",start_code, byte_count, address,
